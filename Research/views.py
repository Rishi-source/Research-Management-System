...
def dashboard(request):
    ...
    completed_projects_count = EndorsementForm.objects.filter(status='completed').count()

    
    ongoing_sanctioned_total = Budget.objects.filter(project__status='ongoing').aggregate(
        total_sanctioned=Sum(
            ExpressionWrapper(
                F('Equipment') + F('Consumables') + F('Fellowship') + F('Contingency') + F('Travel') + F('Field_Testing') + F('Miscellaneous'),
                output_field=DecimalField()
            )
        )
    )['total_sanctioned'] or 0

    completed_sanctioned_total = Budget.objects.filter(project__status='completed').aggregate(
        total_sanctioned=Sum(
            ExpressionWrapper(
                F('Equipment') + F('Consumables') + F('Fellowship') + F('Contingency') + F('Travel') + F('Field_Testing') + F('Miscellaneous'),
                output_field=DecimalField()
...
def add_pi_view(request):
    ...
    return render(request, 'management/add_pi.html', {
...
def add_budget_view(request, project_id):
    ...
    return render(request, 'management/add_sanctioned_amount.html', {'project': project, 'budget': budget})
...
def add_received_amount_view(request, project_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request, f"An error occurred while saving the received amount: {e}")
        
        return redirect('project_list')  

    return render(request, 'management/add_received_amount.html', {'project': project, 'received_amount': received_amount})
...
def add_installment(request, project_id):
    ...
    if request.method == "POST":
        ...
        if installments.filter(installment_year_id=installment_year_id).exists():
            ...
            Installment.objects.create(project=project, installment_year=financial_year, amount=amount)
            messages.success(request, "Installment added successfully.")
            return redirect('add_installment', project_id=project.id)

    financial_years = FinancialYear.objects.exclude(id__in=installments.values_list('installment_year_id', flat=True))
    context = {
        'project': project,
...
def edit_installment(request, project_id, installment_id):
    if not request.user.is_authenticated:
        return redirect('login')
    installment = get_object_or_404(Installment, id=installment_id, project_id=project_id)
...
def edit_expenditure(request, expenditure_id):
    ...
    financial_years = FinancialYear.objects.all()
    expenditure_types = [
        "Equipment", "Consumables", "Fellowship", 
        "Contingency", "Travel", "Field Testing", "Miscellaneous"
    ]
...
def edit_project(request, project_id):
    ...
    if request.method == 'POST':
        ...
        endorsement.Project_Title = request.POST.get('Project_Title', endorsement.Project_Title)
        endorsement.PSRN = request.POST.get('PSRN', endorsement.PSRN)
        endorsement.Name_PI = request.POST.get('Name_PI', endorsement.Name_PI)
        endorsement.Designation_PI = request.POST.get('Designation_PI', endorsement.Designation_PI)
        ...
        endorsement.project_type = request.POST.get('project_type', endorsement.project_type)
        endorsement.funding_agency = request.POST.get('funding_agency', endorsement.funding_agency)
        endorsement.funding_scheme = request.POST.get('funding_scheme', endorsement.funding_scheme)
        endorsement.funding_agency_type = request.POST.get('funding_agency_type', endorsement.funding_agency_type)
        endorsement.CAPEX = request.POST.get('CAPEX', endorsement.CAPEX)
        endorsement.OPEX = request.POST.get('OPEX', endorsement.OPEX)
        endorsement.project_duration = request.POST.get('project_duration', endorsement.project_duration)

        
        commencement_date = request.POST.get('commencement_date_of_project')
        endorsement.commencement_date_of_project = commencement_date if commencement_date else None
    ...
    financial_years = FinancialYear.objects.all()
    co_pi_ids = [
        endorsement.Co_PI1_id, endorsement.Co_PI2_id, endorsement.Co_PI3_id,
        endorsement.Co_PI4_id, endorsement.Co_PI5_id, endorsement.Co_PI6_id
    ]
...
def project_details(request, project_id):
    if not request.user.is_authenticated:
        return redirect('login')
    project = get_object_or_404(EndorsementForm, id=project_id)
    principal_investigators = PrincipleInvestigator.objects.filter(id__in=[
        project.Co_PI1_id, project.Co_PI2_id, project.Co_PI3_id, 
        project.Co_PI4_id, project.Co_PI5_id, project.Co_PI6_id
    ]).exclude(id=None)
    ...
    types = ['Equipment', 'Consumables', 'Fellowship', 'Contingency', 'Travel', 'Field Testing', 'Miscellaneous']
    total_by_type = {exp_type: expenditures.filter(expenditure_type=exp_type).aggregate(Sum('amount'))['amount__sum'] or 0 for exp_type in types}


    
    total_sanctioned = budget.Equipment + budget.Consumables + budget.Fellowship + budget.Contingency + budget.Travel + budget.Field_Testing + budget.Miscellaneous if budget else 0
    total_received = received_amount.Equipment + received_amount.Consumables + received_amount.Fellowship + received_amount.Contingency + received_amount.Travel + received_amount.Field_Testing + received_amount.Miscellaneous if received_amount else 0
    total_installments_amount = installments.aggregate(Sum('amount'))['amount__sum'] or 0
    total_expenditure = expenditures.aggregate(Sum('amount'))['amount__sum'] or 0

    
    balance_equipment_sanctioned = (budget.Equipment if budget else 0) - (expenditures.filter(expenditure_type='Equipment').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_consumables_sanctioned = (budget.Consumables if budget else 0) - (expenditures.filter(expenditure_type='Consumables').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_fellowship_sanctioned = (budget.Fellowship if budget else 0) - (expenditures.filter(expenditure_type='Fellowship').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_contingency_sanctioned = (budget.Contingency if budget else 0) - (expenditures.filter(expenditure_type='Contingency').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_travel_sanctioned = (budget.Travel if budget else 0) - (expenditures.filter(expenditure_type='Travel').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_field_testing_sanctioned = (budget.Field_Testing if budget else 0) - (expenditures.filter(expenditure_type='Field Testing').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_miscellaneous_sanctioned = (budget.Miscellaneous if budget else 0) - (expenditures.filter(expenditure_type='Miscellaneous').aggregate(Sum('amount'))['amount__sum'] or 0)

    balance_equipment_received = (received_amount.Equipment if received_amount else 0) - (expenditures.filter(expenditure_type='Equipment').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_consumables_received = (received_amount.Consumables if received_amount else 0) - (expenditures.filter(expenditure_type='Consumables').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_fellowship_received = (received_amount.Fellowship if received_amount else 0) - (expenditures.filter(expenditure_type='Fellowship').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_contingency_received = (received_amount.Contingency if received_amount else 0) - (expenditures.filter(expenditure_type='Contingency').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_travel_received = (received_amount.Travel if received_amount else 0) - (expenditures.filter(expenditure_type='Travel').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_field_testing_received = (received_amount.Field_Testing if received_amount else 0) - (expenditures.filter(expenditure_type='Field Testing').aggregate(Sum('amount'))['amount__sum'] or 0)
    balance_miscellaneous_received = (received_amount.Miscellaneous if received_amount else 0) - (expenditures.filter(expenditure_type='Miscellaneous').aggregate(Sum('amount'))['amount__sum'] or 0)
...
def manage_departments(request):
    if not request.user.is_authenticated:
        return redirect('login')
    departments = Department.objects.all()
    return render(request, 'management/add_departments.html', {'departments': departments})
...
def manage_organizations(request):
    ...
    return render(request, 'management/add_organisations.html', {'organizations': organizations})
...
def manage_financial_years(request):
    if not request.user.is_authenticated:
        return redirect('login')
    financial_years = FinancialYear.objects.all()
    return render(request, 'management/add_financial_year.html', {'financial_years': financial_years})
...
def project_details_pdf(request, project_id):
    ...
    types = ['Equipment', 'Consumables', 'Fellowship', 'Contingency', 'Travel', 'Field Testing', 'Miscellaneous']
    attribute_mapping = {
        'Field Testing': 'Field_Testing'
    }

    total_by_type = {
        exp_type: expenditures.filter(expenditure_type=exp_type).aggregate(Sum('amount'))['amount__sum'] or 0 for exp_type in types
    }

    total_sanctioned = sum([
        getattr(budget, attribute_mapping.get(type_, type_), 0) or 0 for type_ in types
    ]) if budget else 0

    total_received = sum([
        getattr(received_amount, attribute_mapping.get(type_, type_), 0) or 0 for type_ in types
    ]) if received_amount else 0
    ...
    balance_by_type = {
        f"balance_{type_.lower().replace(' ', '_')}_sanctioned": (getattr(budget, attribute_mapping.get(type_, type_)) if budget else 0) - total_by_type[type_]
        for type_ in types
    }
    balance_by_type.update({
        f"balance_{type_.lower().replace(' ', '_')}_received": (getattr(received_amount, attribute_mapping.get(type_, type_)) if received_amount else 0) - total_by_type[type_]
        for type_ in types
    })
...
def add_project(request):
    ...
    if request.method == 'POST':
        ...
        if not all([project_title, psrn, name_pi, designation_pi, organization_id, department_id, project_type, funding_agency, financial_year_id, capex, opex, project_duration]):
        ...
        except (Organization.DoesNotExist, Department.DoesNotExist, FinancialYear.DoesNotExist):
            messages.error(request, 'Invalid organization, department, or financial year selected.')
            return redirect('add_project')
        ...
        if co_pis.exists():
            if len(co_pis) > 0: endorsement.Co_PI1 = co_pis[0]
            if len(co_pis) > 1: endorsement.Co_PI2 = co_pis[1]
            if len(co_pis) > 2: endorsement.Co_PI3 = co_pis[2]
            if len(co_pis) > 3: endorsement.Co_PI4 = co_pis[3]
            if len(co_pis) > 4: endorsement.Co_PI5 = co_pis[4]
            if len(co_pis) > 5: endorsement.Co_PI6 = co_pis[5]
...
def bifurcate_budget(request, project_id, installment_id):
    ...
    budget, created = FinYearBudget.objects.get_or_create(project=project, year=installment.installment_year)
